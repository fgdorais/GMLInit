...
@[simp, clean, elim_casts] theorem eqRec_heq_iff_heq {α} {a : α} {β : (a' : α) → a = a' → Sort _} {a' : α} (h : a = a') (x : β a rfl) (y : γ) : (Eq.rec x h : β a' h) ≅ y ↔ x ≅ y := by cases h; exact Iff.rfl

@[simp, clean, elim_casts] theorem heq_eqRec_iff_heq {α} {a : α} {β : (a' : α) → a = a' → Sort _} {a' : α} (h : a = a') (x : β a rfl) (y : γ) : y ≅ (Eq.rec x h : β a' h) ↔ y ≅ x := by cases h; exact Iff.rfl
