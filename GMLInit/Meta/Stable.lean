...
syntax "by_contradiction" withPosition(colGe ("|" "assuming" (ident <|> "_") "=>" (hole <|> syntheticHole <|> tacticSeq))) : tactic
macro_rules
| `(tactic|by_contradiction | assuming $h => $t) =>
  `(tactic|apply Stable.by_contradiction _; intro $h; $t)
...
syntax "by_no_counterexample" term:51 " = " ident withPosition(colGe ("|" "assuming" (ident <|> "_") "=>" (hole <|> syntheticHole <|> tacticSeq))) : tactic
macro_rules
