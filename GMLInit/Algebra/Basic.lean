...
structure RingSig (α) extends SemiringSig α where
  neg : α → α
  zero : α
...
unif_hint {α} (s : SemigroupSig α) (t : RingSig α) where
  s =?= t.toAddGroupSig.toSemigroupSig ⊢ s =?= t.toSemiringSig.toAddSemigroupSig
...
structure UnitalRingSig (α) extends RingSig α, UnitalSemiringSig α
...
section Identities
variable {α} (op : α → α → α) (op' : outParam (α → α → α)) (inv : outParam (α → α)) (id : outParam α)
