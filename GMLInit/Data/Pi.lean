...
protected def equiv {α₁ α₂} {β₁ : α₁ → Sort _} {β₂ : α₂ → Sort _} (e : Equiv α₁ α₂) (f : (x₁ : α₁) → Equiv (β₁ x₁) (β₂ (e.fwd x₁))) : Equiv ((x₁ : α₁) → β₁ x₁) ((x₂ : α₂) → β₂ x₂) :=
  let h1 : Equiv ((x₁ : α₁) → β₁ x₁) ((x₂ : α₂) → β₁ (e.rev x₂))
    := equivFst β₁ e
  let h2 : Equiv ((x₂ : α₂) → β₁ (e.rev x₂)) ((x₂ : α₂) → β₂ (e.fwd (e.rev x₂)))
    := equivSnd (λ x₂ => f (e.rev x₂))
  Equiv.comp h (Equiv.comp h2 h1)
  where h := {
